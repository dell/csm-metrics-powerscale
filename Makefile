...
podman: download-csm-common
	...
	podman build --pull $(NOCACHE) -t csm-metrics-powerscale -f Dockerfile --build-arg BASEIMAGE=$(CSM_BASEIMAGE) --build-arg GOIMAGE=$(DEFAULT_GOIMAGE) .
...
.PHONY: push
push:
	podman push ${DOCKER_REPO}/csm-metrics-powerscale\:latest

.PHONY: tag
tag:
	podman tag csm-metrics-powerscale\:latest ${DOCKER_REPO}/csm-metrics-powerscale\:latest
